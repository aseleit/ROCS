% Duffing Oscillator Problem - x10-x2f
function [problem,guess] = DuffingOscillator4
%Duffing Oscillator Problem - Fixed Final State
%
...
InternalDynamics=@DuffingOscillator_Dynamics_Internal;
SimDynamics=@DuffingOscillator_Dynamics_Sim;
...
% Settings file
problem.settings=@settings_DuffingOscillator;
...
problem.states.x0l=[0 -0.7900]; 
problem.states.x0u=[0 10]; 
...
problem.states.xErrorTol_integral=[1e-15 1e-15];

% State constraint error bounds
problem.states.xConstraintTol=[1e-15 1e-15];


% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[1.4063 2]; 
problem.states.xfu=[5 2];

% Guess the state trajectories with [x0 xf]
guess.states(:,1)=[0 1.4063];
guess.states(:,2)=[-0.7900 2];
...
problem.inputs.uConstraintTol=[1e-15];

% Guess the input sequences with [u0 uf]
guess.inputs(:,1)=[0, 1.52];   
...
% Bounds for path constraint function gl =<  (x,u,p,t) =< gu
...
x1f = x(end,1);
x2f = x(end,2);
% stageCost =  0.5*(x1f.*x1f + x2f.*x2f) + 0.5*u(:,1).*u(:,1);
stageCost =   0.5*u(:,1).*u(:,1); 
