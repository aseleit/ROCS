...
options= problem.settings(1,5);                  % Get options and solver settings
...
switch BCtype 
    ...
    case 'P0-Pf'
        initial_guess_newton = [2.3*ones(2*N,1); 2.3*ones(2*N,1)];
        [resid,JacB] = duffingNAE(initial_guess_newton,BC,omega,beta,D,N,BCtype);
        solu = initial_guess_newton;
    case 'P0-Vf'
...
switch BCtype
    case "fixed"
        ...
        plot(t,Xexact(:,2),'rd')
        plot(t,-Xexact(:,4),'rs')
        legend({'x1(CRBF)','x2(CRBF)','u(CRBF)','x1(ode45)','x2(ode45)','u(ode45)'},'Location','northwest')
        axis('tight')
        xlabel('Time [sec]')
